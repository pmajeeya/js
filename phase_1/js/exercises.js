...
const getPositiveIncomeAmount = (data) => {
	return data.filter((v) => v.amount >= 0)
		.reduce((acc, v) => (typeof(acc) === 'object' ? acc.amount : acc) + v.amount);
};
console.log(getPositiveIncomeAmount(funds));

const getTotalIncomeAmount = (data) => {
	console.log(data.every((v) => v.amount > 0));
	if (data.every((v) => v.amount > 0)){
		return getPositiveIncomeAmount(data);
	} else {
		return data.reduce((acc, v) => (typeof(acc) === 'object' ? acc.amount : acc) + v.amount);;
	}
};
